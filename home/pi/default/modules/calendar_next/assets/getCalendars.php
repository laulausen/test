<?php
	...
	if(empty($calendarString)) {
		...
		if(is_null($calendars)) {
			...
			setConfigValue('calendar_next_calendars', json_encode($calendars));
		}
	}
	...
	echo json_encode(parseCalendars(get_calendar_by_urls($calendars), (int)$calendar_next_showToday) );
...
//							"Accept-Encoding: gzip,deflate,sdchrn" .
	...
	function parseCalendars($calendars, $showToday) {
		$calendarRows = explode("\n", $calendars);
					...
					if(isset($event['rrule'])) {
							...
							if($isFullTime) {
								for($i = 0; $i < 5; $i++) {
									...
									if($endDate <= $until || $until == -1) {
										...
										while($startDate < $endDate) {
											if((date('Ymd', $startDate) >= date('Ymd') && $showToday) || (date('Ymd', $startDate) > date('Ymd') && !$showToday)) {
												$save['title'] = $event['title'];
												$save['fulltime'] = true;
												$future[$startDate][] = $save;
											}
						...
						if($isFullTime) {
							...
							if($endDate	> time()) {
								...
								while($date < $endDate) {
									if((date('Ymd', $date) >= date('Ymd') && $showToday) || (date('Ymd', $date) > date('Ymd') && !$showToday)) {
										$save['title'] = $event['title'];
										$save['fulltime'] = true;
										$future[$date][] = $save;
									}
							...
							if($startDateTstamp >= time()) {
								if((substr($event['start'],0,8) >= date('Ymd') && $showToday) || (substr($event['start'],0,8) > date('Ymd') && !$showToday)) {
									$save['title'] = $event['title'];
									$save['fulltime'] = false;
									$future[$startDateTstamp][] = $save;
								}
							}
						}
					}
		...
		$future = array_slice($future, 0 , 5, true);
		
		foreach($future as $date => $events) {
			$outDate = date('d.m.Y', $date);
