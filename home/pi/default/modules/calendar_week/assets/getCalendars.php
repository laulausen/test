...
//							"Accept-Encoding: gzip,deflate,sdchrn" .
		...
		foreach($someCalendars as $cal)	{  
	        ...
	        $content[$cal['label']] .=	timeShift($precontent, $cal['timeShift']);
		}
	...
	function parseCalendars($calendarArray) {
		foreach($calendarArray as $label => $calendars) {
			$calendarRows = explode("\n", $calendars);
						...
						if(isset($event['rrule'])) {
								...
								if($isFullTime) {
									for($i = 0; $i < 5; $i++) {
										...
										if($endDate <= $until || $until == -1) {
											...
											while($startDate < $endDate) {
												if(date('Ymd', $startDate) >= date('Ymd') && date('Ymd', $startDate) <= date('Ymd', strtotime("+4 days"))) {
													$newEntry['name'] = $event['title'];
													$newEntry['fulltime'] = true;
													$newEntry['label'] = $label;
													$week[$startDate][] = $newEntry;
												} 
							...
							if($isFullTime) {
								...
								if($endDate	> time()) {
									...
									while($date < $endDate) {
										if(date('Ymd', $date) >= date('Ymd') && date('Ymd', $date) <= date('Ymd', strtotime("+4 days"))) {
											$newEntry['name'] = $event['title'];
											$newEntry['fulltime'] = true;
											$newEntry['label'] = $label;
											$week[$date][] = $newEntry;
								...
								if($startDateTstamp > time()) {
									if(substr($event['start'],0,8) >= date('Ymd') && date('Ymd', $startDateTstamp) <= date('Ymd', strtotime("+4 days"))) {
										$newEntry['name'] = $event['title'];
										$newEntry['fulltime'] = false;
										$newEntry['label'] = $label;
										$week[$startDateTstamp][] = $newEntry;
									}
								}
							}
						}
